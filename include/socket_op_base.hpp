 ...
 */
#ifndef SOCKET_OP_BASE_HPP_
#define SOCKET_OP_BASE_HPP_
...
class epoll_context::socket_base_operation
 ...
 public:
  ...
  void add_events() {
    epoll_event event{};
    if constexpr (op_type_ == op_type::op_read ||
                  op_type_ == op_type::op_accept) {
      event.events = EPOLLIN | EPOLLERR | EPOLLHUP | EPOLLPRI | EPOLLET;
    } else {
      event.events = EPOLLOUT | EPOLLERR | EPOLLHUP | EPOLLPRI | EPOLLET;
    }
    event.data.ptr = this;
    ::epoll_ctl(context_.epoll_fd_, EPOLL_CTL_ADD, socket_.native_handle(),
                &event);
  }

  void remove_events() {
    epoll_event event = {};
    ::epoll_ctl(context_.epoll_fd_, EPOLL_CTL_DEL, socket_.native_handle(),
                &event);
  }

  // Prepare.
  socket_base_operation(Receiver&& receiver, socket_type&& socket,
                        const op_vtable& op_vtable, const op_type& op_type)
  ...
  friend void tag_invoke(stdexec::start_t,
                         socket_base_operation& self) noexcept {
    if (!self.context_.is_running_on_io_thread()) {
      static_cast<completion_op*>(self)->execute_ =
          &socket_base_operation::on_schedule_complete;
  ...
  static void wakeup(operation_base* op) noexcept {
    ...
    if ((self.state_ & socket_base_operation::request_stopped_mask) != 0) {
      ...
      // and set stopped to downstream receiver and deregister peer socket fd
      // from epoll.
      return;
    }
    ...
    auto old_state = self.state_.fetch_add(
        socket_base_operation::operation_ended, std::memory_order_acq_rel);
  ...
  static void complete_with_stop(operation_base* op) noexcept {
    ...
    auto& self =
        *static_cast<socket_base_operation*>(static_cast<stop_op*>(op));
    if (!static_cast<completion_op&>(self).enqueued_.load()) {
      ...
      // The current operation has been committed to the queue, we do not offer
      // to delete the operation from the queue, so wait for the operation to be
      // completed, then the stop operation will be executed sequentially, at
      // which point the operation will be stopped, and no further completion
      // operation will be committed to the queue.
  ...
  void request_stop() noexcept {
    ...
    if ((old_state & socket_base_operation::operation_ended_mask) == 0) {
      ...
      // epoll_event event = {};
      // (void)::epoll_ctl(context_.epoll_fd_, EPOLL_CTL_DEL, socket_->fd(),
      // &event);
 ...
 private:
  ...
  // TODO: 放到io里
  // Total number of bytes transferred, to be passed to the downstream receiver
  // when this operation is successfully executed.
  ...
  // The state to record this operation is executing io operation or has been
  // cancelled.
