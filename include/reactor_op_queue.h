#ifndef SRC_DETAIL_REACTOR_OP_QUEUE_HPP
#define SRC_DETAIL_REACTOR_OP_QUEUE_HPP
...
class reactor_op_queue : private NonCopyable {
 public:
  ...
  bool enqueue_operation(Descriptor descriptor, ReactorOp* op) {
  ...
  bool cancel_operations(iterator i, op_queue<operation>& ops,
                         const std::error_code& ec = asio::error::operation_aborted) {
    if (i != operations_.end()) {
      while (ReactorOp* op = i->second.front()) {
        op->ec_ = ec;
        i->second.pop();
        ops.push(op);
      }
  ...
  bool cancel_operations_by_key(iterator i, op_queue<operation>& ops, void* cancellation_key,
    ...
    if (i != operations_.end()) {
      ...
      if (i->second.empty()) operations_.erase(i);
    }
    return result;
  }
...
#endif  // SRC_DETAIL_REACTOR_OP_QUEUE_HPP
