...
class const_buffer {
 public:
  ...
  constexpr const_buffer(const mutable_buffer& b) noexcept  // NOLINT
...
inline constexpr const
    mutable_buffer* buffer_sequence_begin(const MutableBuffer& b) noexcept {
...
inline constexpr const
    const_buffer* buffer_sequence_begin(const ConstBuffer& b) noexcept {
...
template <typename MutableBuffer>
  requires(std::convertible_to<const MutableBuffer*, const mutable_buffer*>)
inline constexpr const
    mutable_buffer* buffer_sequence_end(const MutableBuffer& b) noexcept {
...
template <typename ConstBuffer>
  requires(std::convertible_to<const ConstBuffer*, const const_buffer*>)
inline constexpr const
    const_buffer* buffer_sequence_end(const ConstBuffer& b) noexcept {
...
inline constexpr std::size_t buffer_size(one_buffer, Iterator begin,
                                         Iterator) noexcept {
  return const_buffer{*begin}.size();
}

template <typename Iterator>
inline constexpr std::size_t buffer_size(multiple_buffers, Iterator begin,
...
inline constexpr mutable_buffer operator+(const mutable_buffer& b,
...
// Create a new modifiable buffer that is offset from the start of another.
inline constexpr mutable_buffer operator+(std::size_t n,
                                          const mutable_buffer& b) noexcept {
  return b + n;
}

// Create a new non-modifiable buffer that is offset from the start of another.
inline constexpr const_buffer operator+(const const_buffer& b,
                                        std::size_t n) noexcept {
  std::size_t offset = n < b.size() ? n : b.size();
...
// Create a new non-modifiable buffer that is offset from the start of another.
inline constexpr const_buffer operator+(std::size_t n,
...
// Create a new modifiable buffer from an existing buffer.
inline constexpr mutable_buffer buffer(const mutable_buffer& b,
                                       std::size_t max_size) noexcept {
  return mutable_buffer{b.data(), b.size() < max_size ? b.size() : max_size};
}

// Create a new non-modifiable buffer from an existing buffer.
inline constexpr const_buffer buffer(const const_buffer& b) noexcept {
  return const_buffer{b};
}

// Create a new non-modifiable buffer from an existing buffer.
inline constexpr const_buffer buffer(const const_buffer& b,
                                     std::size_t max_size) noexcept {
...
// Create a new modifiable buffer that represents the given POD array.
template <typename PodType, std::size_t N>
inline constexpr mutable_buffer buffer(PodType (&data)[N],
                                       std::size_t max_size) noexcept {
  return mutable_buffer{
      data, N * sizeof(PodType) < max_size ? N * sizeof(PodType) : max_size};
}
...
// Create a new non-modifiable buffer that represents the given POD array.
template <typename PodType, std::size_t N>
inline constexpr const_buffer buffer(const PodType (&data)[N],
                                     std::size_t max_size) noexcept {
  return const_buffer{
      data, N * sizeof(PodType) < max_size ? N * sizeof(PodType) : max_size};
}
...
inline constexpr mutable_buffer buffer(std::array<PodType, N>& data,
                                       std::size_t max_size) noexcept {
  return {data.data(), data.size() * sizeof(PodType) < max_size
...
inline constexpr mutable_buffer buffer(
...
inline constexpr mutable_buffer buffer(std::vector<PodType, Allocator>& data,
                                       std::size_t max_size) noexcept {
  return {data.size() ? &data[0] : 0, data.size() * sizeof(PodType) < max_size
                                          ? data.size() * sizeof(PodType)
                                          : max_size};
}
...
inline constexpr mutable_buffer buffer(
    ...
    std::size_t max_size) noexcept {
  return {data.size() ? &data[0] : 0, data.size() * sizeof(Elem) < max_size
...
inline constexpr const_buffer buffer(
    ...
    std::size_t max_size) noexcept {
  return {data.data(), data.size() * sizeof(Elem) < max_size
                           ? data.size() * sizeof(Elem)
                           : max_size};
}
...
concept mutable_contiguous_container =
    ...
    (!std::is_const<
        typename std::remove_reference<typename std::iterator_traits<
            typename T::iterator>::reference>::type>::value);

template <typename T>
concept const_contiguous_container =
    std::contiguous_iterator<typename T::iterator> &&
    (std::is_const<typename std::remove_reference<typename std::iterator_traits<
         typename T::iterator>::reference>::type>::value);
...
inline constexpr const_buffer
    buffer(const T& data, std::size_t max_size) noexcept {
  return const_buffer(data.size() ? std::addressof(data.begin()) : 0,
                      data.size() * sizeof(typename T::value_type) < max_size
                          ? data.size() * sizeof(typename T::value_type)
                          : max_size);
...
class dynamic_string_buffer {
 public:
  ...
  // Get a sequence of buffers that represents the underlying memory.
  constexpr const_buffers_type data(std::size_t pos,
  ...
  // Remove characters from the input sequence. Consume the specified number of
  // bytes from the beginning of the underlying memory.
...
std::size_t buffer_copy(one_buffer,                   //
  ...
  for (mutable_buffer target{buffer(*target_begin, max_bytes_to_copy)};
       target.size() && source_iter != source_end;  //
       ++source_iter) {
    const_buffer source{*source_iter};
    std::size_t bytes_copied = buffer_copy_1(target, source);
...
std::size_t buffer_copy(multiple_buffers,             //
  ...
  for (const_buffer source{buffer(*source_begin, max_bytes_to_copy)};
       source.size() && target_iter != target_end;  //
       ++target_iter) {
    mutable_buffer target{*target_iter};
    std::size_t bytes_copied = buffer_copy_1(target, source);
    total_bytes_copied += bytes_copied;
    source += bytes_copied;
  }
...
std::size_t buffer_copy(multiple_buffers,             //
  ...
  while (target_iter != target_end && source_iter != source_end) {
    mutable_buffer target{mutable_buffer(*target_iter) + target_buffer_offset};
    const_buffer source{const_buffer(*source_iter) + source_buffer_offset};
    std::size_t bytes_copied = buffer_copy_1(target, source);
    total_bytes_copied += bytes_copied;

    if (bytes_copied == target.size()) {
      ++target_iter;
...
std::size_t buffer_copy(multiple_buffers,             //
  ...
  while (total_bytes_copied != max_bytes_to_copy && target_iter != target_end &&
    ...
    std::size_t bytes_copied = buffer_copy_1(
        target, buffer(source, max_bytes_to_copy - total_bytes_copied));
    total_bytes_copied += bytes_copied;
...
// buffer sequence. Return the bytes that real copied.
template <typename MutableBufferSequence, typename ConstBufferSequence>
inline std::size_t buffer_copy(const MutableBufferSequence& target,
...
// Concept to determine whether a type satisfies the MutableBufferSequence
// requirements.
template <typename T>
concept mutable_buffer_sequence =
    ...
    requires(const T& t) {
      ...
      requires std::convertible_to<decltype(*buffer_sequence_begin(t)),
                                   mutable_buffer>;
    };  // NOLINT

// Concept to determine whether a type satisfies the ConstBufferSequence
// requirements.
template <typename T>
