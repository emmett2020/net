...
#ifndef ATOMIC_INTRUSIVE_QUEUE_HPP_
#define ATOMIC_INTRUSIVE_QUEUE_HPP_
...
// An intrusive queue that supports multiple threads concurrently enqueueing
// items to the queue and a single consumer dequeuing items from the queue. The
// consumer always dequeues all items at once, resetting the queue back to
// empty. The consumer can also mark itself as inactive. The next producer to
// enqueue an item will then be notified that the consumer is inactive by the
// return-value of enqueue(). This producer is  then responsible for waking up
// the consumer.
...
class atomic_intrusive_queue<Next> {
 public:
  ...
  // Returns true if previous state was inactive, this operation successfully
  // marked it as active. Returns false if the previous state was active.
  [[nodiscard]] bool try_mark_active() noexcept {
    void* old_value = inactive_value();
    return head_.compare_exchange_strong(old_value, nullptr,
                                         std::memory_order_acquire,
                                         std::memory_order_relaxed);
  }

  // Either enqueue an item to the queue if the producer is active, otherwise if
  // the producer is inactive then mark the queue as active but do not enqueue
  // the item. The assumption is that the caller then becomes the producer and
  // will process item directly.  Returns true if the item was enqueued. Returns
  // false if the item was not enqueued and the queue was transitioned from
  // inactive to active.
  [[nodiscard]] bool enqueue_or_mark_active(Item* item) noexcept {
    ...
    } while (!head_.compare_exchange_weak(old_value, new_value,
                                          std::memory_order_acq_rel));
    return old_value != inactive;
  }

  // Enqueue an item to the queue. Returns true if the producer is inactive and
  // needs to be woken up. The calling thread has responsibility for waking up
  // the producer.
  [[nodiscard]] bool enqueue(Item* item) noexcept {
    ...
    do {
      item->*Next =
          (old_value == inactive) ? nullptr : static_cast<Item*>(old_value);
    } while (!head_.compare_exchange_weak(old_value, item,
                                          std::memory_order_acq_rel));
    return old_value == inactive;
  }
  ...
  [[nodiscard]] stdexec::__intrusive_queue<Next> dequeue_all() noexcept {
    ...
    return stdexec::__intrusive_queue<Next>::make_reversed(
  ...
  [[nodiscard]] bool try_mark_inactive() noexcept {
    ...
    if (old_value == nullptr) {
      if (head_.compare_exchange_strong(old_value, inactive,
  ...
  [[nodiscard]] stdexec::__intrusive_queue<Next>
...
#endif  // ATOMIC_INTRUSIVE_QUEUE_HPP_
