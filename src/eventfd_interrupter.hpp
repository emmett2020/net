#ifndef NET_SRC_EVENTFD_INTERRUPTER_HPP_
#define NET_SRC_EVENTFD_INTERRUPTER_HPP_
...
// Use eventfd to wake up blocking calls. When creating an eventfd fails, a pipe is used to create a
// file descriptor
class eventfd_interrupter {
 ...
 private:
  ...
  // If the values of the two descriptors are the same,  the eventfd is successfully created.
  // If the values of the two descriptors are different, the pipe fd is successfully created.
  ...
  constexpr void close_descriptors() {
    if (write_descriptor_ != invalid_socket_fd && write_descriptor_ != read_descriptor_) {
  ...
  // descriptor is passed to select/epoll etc such that when it is time to stop, a single
  // 64bit value will be written on the other end of the connection and this
  // descriptor will become readable.
  int read_descriptor_;

  // The write end of a connection used to interrupt the blocking call. A single
  // 64bit non-zero value may be written to this to wake up the select/epoll which is
...
#endif  // NET_SRC_EVENTFD_INTERRUPTER_HPP_
