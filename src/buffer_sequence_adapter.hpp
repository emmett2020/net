...
#ifndef NET_SRC_BUFFER_SEQUENCE_ADAPTER_HPP_
#define NET_SRC_BUFFER_SEQUENCE_ADAPTER_HPP_
...
class buffer_sequence_adapter_base {
 ...
 protected:
  ...
  static constexpr void init_native_buffer(iovec& iov, const mutable_buffer& buf) {
    init_iov_base(iov.iov_base, buf.data());
    iov.iov_len = buf.size();
  }

  static constexpr void init_native_buffer(iovec& iov, const const_buffer& buf) {
...
class buffer_sequence_adapter : private buffer_sequence_adapter_base {
 public:
  ...
  explicit constexpr buffer_sequence_adapter(const Buffers& sequence) noexcept
    ...
    buffer_sequence_adapter::init(buffer_sequence_begin(sequence), buffer_sequence_end(sequence));
  }
  ...
  constexpr std::size_t total_size() const noexcept { return total_buffer_size_; }
  ...
  static Buffer linearise(const Buffers& sequence, const mutable_buffer& storage) {
    return buffer_sequence_adapter::linearise(buffer_sequence_begin(sequence),
                                              buffer_sequence_end(sequence), storage);
  }
 ...
 private:
  ...
  template <typename Iterator>
  static Buffer linearise(Iterator begin, Iterator end, const mutable_buffer& storage) {
...
class buffer_sequence_adapter<Buffer, mutable_buffer> : buffer_sequence_adapter_base {
 public:
  ...
  static constexpr Buffer linearise(const mutable_buffer& sequence, const Buffer&) noexcept {
    return Buffer(sequence);
  }
...
template <typename Buffer>
class buffer_sequence_adapter<Buffer, const_buffer> : buffer_sequence_adapter_base {
 public:
  static constexpr bool is_single_buffer = true;
  static constexpr int linearization_storage_size = 1;
  ...
  static constexpr void validate(const const_buffer& sequence) noexcept { sequence.data(); }

  static constexpr Buffer first(const const_buffer& sequence) noexcept { return Buffer(sequence); }

  static constexpr Buffer linearise(const const_buffer& sequence, const Buffer&) noexcept {
    return Buffer(sequence);
  }
...
class buffer_sequence_adapter<Buffer, std::array<Elem, 2>> : buffer_sequence_adapter_base {
 public:
  ...
  explicit constexpr buffer_sequence_adapter(const std::array<Elem, 2>& sequence) noexcept {
  ...
  static constexpr bool all_empty(const std::array<Elem, 2>& sequence) noexcept {
...
#endif  // NET_SRC_BUFFER_SEQUENCE_ADAPTER_HPP_
