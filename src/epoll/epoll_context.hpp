#ifndef SRC_NET_EPOLL_EPOLL_CONTEXT_HPP_
#define SRC_NET_EPOLL_EPOLL_CONTEXT_HPP_
...
namespace net {
namespace __epoll {
// The usage mode is single thread single epoll. Multithreading drive one context is not allowed.
// The thread which running the context is called io thread, and others are called remote threads.
class epoll_context final : public execution_context {
 public:
  // The scheduler of this context. Both `schedule_at`, `schedule_after` and `schedule`
  // customization point object are supported by this scheduler.
  class scheduler;

  // The base class for all the types of operations that this context can perform.
  // Note that operations will be executed by context in the order they are committed.
  struct operation_base {
    // Default constructor.
    constexpr operation_base() noexcept : enqueued_(false), next_(nullptr), execute_(nullptr) {}

    // Destructor.
    ~operation_base() { assert(!enqueued_); }

    // The flag determines whether the current operation is in a remote or local queue.
  ...
  struct schedule_at_base_op : operation_base {
    ...
    static constexpr std::uint32_t timer_elapsed = 1;
    static constexpr std::uint32_t cancel_pending = 2;
  ...
  // Request to stop the context. Note that the context may block on the epoll_wait call, so we
  // must use interrupt to wake up the context.
  void request_stop() {
  ...
  stdexec::in_place_stop_token get_stop_token() const noexcept { return stop_source_->get_token(); }

  // Check whether this context is running.
  bool is_running() const noexcept { return is_running_.load(std::memory_order_relaxed); }

  // CPO: get_scheduler.
  constexpr scheduler get_scheduler() noexcept;

 private:
  // The thread that calls `context.run()` is called io thread, and other threads are remote
  // threads. This function checks which thread is using the context.
  bool is_running_on_io_thread() const noexcept;

  // Check whether the thread submitting an operation to the context is the io thread
  // If the operation is submitted by the io thread, the operation is submitted to
  // the local queue; otherwise, the operation is submitted to remote queue.
  ...
  // Insert the timer operation into the queue of timers. Must be called from the I/O thread.
  void schedule_at_impl(schedule_at_base_op* op) noexcept;

  // Execute all items on the local queue.
  // Won't run other items that were enqueued during the execution of the items that were already
  // enqueued. This bounds the amount of work to a finite amount.
  size_t execute_local() noexcept;

  // Check if any completion queue items are available and if so add them to the local queue.
  void acquire_completion_queue_items();

  // Collect the contents of the remote queue and pass them to local queue.
  // The return value represents whether the remote queue is empty before we make a collection.
  // Returns true means remote queue is emtpy before we collect.
  bool try_schedule_remote_to_local() noexcept;

  // Signal the remote queue eventfd.
  // This should only be called after trying to enqueue() work to the remote queue and being told
  // that the I/O thread is inactive.
  ...
  // `due_time` indicates the absolute time since system startup. This timer will only alarm once.
  // If both due_time.seconds() and due_time.nanoseconds() are zero, this timer would't alarm.
  ...
  // Add the timer's file descriptor to epoll. Throws an error when registrations fails.
  void add_timer_to_epoll();

  // Remove the timer's file descriptor from epoll. Throws an error when remove fails.
  void remove_timer_from_epoll();

  // Add the interrupter's file descriptor to epoll.  Throws an error when registrations fails.
  void add_interrupter_to_epoll();

  // Remove the interrupter's file descriptor from epoll. Throws an error when remove fails.
  ...
  // Notifying a remote thread to wake up from `epoll_wait`, usually after committing an operation
  // to another thread, allows the other thread to execute the operation immediately without
  // remaining stuck in `epoll_wait`.
...
// The scheduler with returned by `stdexec::get_schedule` customization point object.
class epoll_context::scheduler {
  ...
  // The sender returned by either `stdexec::schedule_at` or `stdexec::schedule_after` customization
  // point object.
  class schedule_at_sender;

  //                                                                                     [implement]
  struct schedule_env {
    friend auto tag_invoke(stdexec::get_completion_scheduler_t<stdexec::set_value_t>,
                           const schedule_env& env) noexcept -> scheduler {
      return scheduler{env.context};
    }

    explicit constexpr schedule_env(epoll_context& ctx) noexcept : context(ctx) {}
  ...
  class schedule_op {
   ...
   public:
    struct __t : private operation_base {
      ...
      friend void tag_invoke(stdexec::start_t, __t& op) noexcept { op.start_impl(); }
  ...
  class schedule_sender {
   ...
   public:
    struct __t {
      ...
      friend auto tag_invoke(stdexec::get_completion_signatures_t, const __t& self, Env&&) noexcept
  ...
  class schedule_at_op {
    ...
    struct __t : epoll_context::schedule_at_base_op {
      ...
      constexpr __t(epoll_context& context, const time_point& due_time, receiver_t r) noexcept
     ...
     private:
      ...
      static constexpr void complete_with_stop(operation_base* op) noexcept {
        if constexpr (!stdexec::unstoppable_token<stop_token>) {
          stdexec::set_stopped(static_cast<receiver_t&&>(static_cast<__t*>(op)->receiver_));
        } else {
      ...
      static constexpr void complete_with_value(operation_base* op) noexcept {
        ...
        if constexpr (!stdexec::unstoppable_token<stop_token>) {
          auto stop_token = stdexec::get_stop_token(stdexec::get_env(timer_op.receiver_));
      ...
      static constexpr void remove_timer_and_complete_with_stopped(operation_base* op) noexcept {
        if constexpr (!stdexec::unstoppable_token<stop_token>) {
          auto& timer_op = *static_cast<__t*>(op);
      ...
      constexpr void request_stop_local() noexcept {
        ...
        if ((state & schedule_at_base_op::timer_elapsed) == 0) {
          // Timer not yet elapsed. Remove timer from list of timers and enqueue cancellation.
      ...
      constexpr void request_stop_remote() noexcept {
        auto oldState =
            state_.fetch_add(schedule_at_base_op::cancel_pending, std::memory_order_acq_rel);
        if ((oldState & schedule_at_base_op::timer_elapsed) == 0) {
          // Timer had not yet elapsed.
          // We are responsible for scheduling the completion of this timer operation.
 ...
 public:
  // Constructors.
  explicit constexpr scheduler(epoll_context& context) noexcept : context_(&context) {}
  ...
  friend auto tag_invoke(exec::schedule_at_t,     //
                         ...
                         const time_point& due_time) noexcept -> stdexec::__t<schedule_at_sender> {
...
inline int epoll_context::create_epoll() {
  ...
  if (fd < 0) {
    // If creating an epoll fails, it makes more sense to throw an exception to terminate the
    // program than to give a solution
    throw std::system_error{static_cast<int>(errno), std::system_category(), "epoll_create(1)"};
  }
  return fd;
}
...
inline void epoll_context::add_timer_to_epoll() {
  epoll_event event = {.events = EPOLLIN | EPOLLERR, .data = {.ptr = timers_data()}};
...
inline void epoll_context::add_interrupter_to_epoll() {
  epoll_event ev = {.events = EPOLLIN | EPOLLERR | EPOLLET, .data = {.ptr = &interrupter_}};
  if (::epoll_ctl(epoll_fd_, EPOLL_CTL_ADD, interrupter_.read_descriptor(), &ev) < 0) {
    throw std::system_error{static_cast<int>(errno), std::system_category(),
...
inline void epoll_context::remove_interrupter_from_epoll() {
  epoll_event event = {};
  if (::epoll_ctl(epoll_fd_, EPOLL_CTL_DEL, interrupter_.read_descriptor(), &event) < 0) {
    throw std::system_error{static_cast<int>(errno), std::system_category(),
...
static constexpr std::uint32_t epoll_event_max_count = 256;
...
inline void epoll_context::acquire_completion_queue_items() {
  ...
  for (int i = 0; i < result; ++i) {
    if (events[i].data.ptr == &interrupter_) {
      // No need to read the eventfd to clear the signal since we're leaving the descriptor in a
      // ready-to-read state and relying on edge-triggered notifications. Skip processing this item
      // and let the run loop check for the remote-queued items next time.
      processed_remote_queue_submitted_ = false;
      ...
      if (::read(timer_fd_, &buffer, sizeof(buffer)) < 0) {
        throw std::system_error{static_cast<int>(errno), std::system_category(), "read_timerfd"};
      }
    } else {
      auto& completion_state = *reinterpret_cast<completion_op*>(events[i].data.ptr);
...
inline void epoll_context::run() {
  ...
  exec::scope_guard g{[=]() noexcept { std::exchange(current_thread_context, old_context); }};

  size_t executed_cnt;
  while (true) {
    executed_cnt = execute_local();
    if (stop_source_->stop_requested()) {
      // TODO: What should we do if we want restart the context?
    ...
    if (!processed_remote_queue_submitted_) {
      // This flag will be true if we have collected some items from remote queue to local queue.
      // So we have cleared remote queue. Just wait the next time's interrupt.
...
inline void epoll_context::schedule_local(operation_queue ops) noexcept {
  // Do not adjust the enqueued flag, which is still true because the ops will immediately be
  // transferred from the remote queue to the local queue.
...
inline void epoll_context::update_timers() noexcept {
  ...
  if (!timers_.empty()) {
    ...
    while (!timers_.empty() && timers_.top()->due_time_ <= now) {
      ...
      if (op->can_be_cancelled_) {
        auto old_state =
            op->state_.fetch_add(schedule_at_base_op::timer_elapsed, std::memory_order_acq_rel);
        if ((old_state & schedule_at_base_op::cancel_pending) != 0) {
          // Timer has been cancelled by a remote thread.
          // The other thread is responsible for enqueueing is operation onto the remote_queue_.
          continue;
        }
      }

      // Otherwise, we are responsible for enqueuing the timer onto the ready-to-run queue.
      schedule_local(op);
    }
  }
  ...
  if (timers_.empty()) {
    // If there is no timing operation currently, we should reset the timing time for timerfd
...
inline void epoll_context::set_timer(const time_point& due_time) {
  ::itimerspec time = {
      ...
      .it_value = {.tv_sec = due_time.seconds(), .tv_nsec = due_time.nanoseconds()}};

  if (::timerfd_settime(timer_fd_, TFD_TIMER_ABSTIME, &time, nullptr) < 0) {
    throw std::system_error{static_cast<int>(errno), std::system_category(), "set_timer"};
  }
}
};  // namespace __epoll
...
#endif  // SRC_NET_EPOLL_EPOLL_CONTEXT_HPP_
