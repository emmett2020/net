#ifndef NET_SRC_IP_ADDRESS_HPP_
#define NET_SRC_IP_ADDRESS_HPP_
...
class address {
 public:
  ...
  constexpr address(const address_v4& addr) noexcept : address_(addr) {}

  // Construct an address from an IPv6 address.
  constexpr address(const address_v6& addr) noexcept : address_(addr) {}
  ...
  constexpr bool is_unspecified() const noexcept {
    return std::visit([](auto&& addr) { return addr.is_unspecified(); }, address_);
  }

  // Determine whether the address is a multicast address.
  constexpr bool is_multicast() const noexcept {
    return std::visit([](auto&& addr) { return addr.is_multicast(); }, address_);
  }

  // Compare two addresses for equality.
  constexpr friend bool operator==(const address& a1, const address& a2) noexcept = default;

  // Compare addresses for ordering.
  constexpr std::partial_ordering operator<=>(address const& other) const noexcept {
    return address_.index() == other.address_.index()
               ? (address_.index() == 0
                      ? ::std::get<0>(this->address_) <=> ::std::get<0>(other.address_)
                      : ::std::get<1>(this->address_) <=> ::std::get<1>(other.address_))
               : ::std::partial_ordering::unordered;
  }

  // Get the Linux-specific form of address.
  inline ::socklen_t native_address(::sockaddr_storage* storage, port_type port) const {
    return ::std::visit([storage, port](auto&& addr) { return addr.native_address(storage, port); },
                        address_);
  }
...
// Create an address from an IPv4 address string in dotted decimal form,
// or from an IPv6 address in hexadecimal notation.
inline address make_address(const std::string& str) { return make_address(str.c_str()); }
