...
static const std::vector<std::string> str_ip{"0.0.0.0", "127.0.0.1", "224.0.0.0",
                                             "120.121.122.123"};
static const std::vector<uint32_t> uint_ip{0, 2130706433, 3758096384, 2021227131};

// These unit tests should work fine on a big-ended machine, but since I don't have a big-ended
// machine, I can't guarantee reliability. If you find a bug on a big-endian machine, you are
// welcome to submit an issue.

TEST_CASE("[default ctor should return unspecified address]", "[address_v4.ctor]") {
...
TEST_CASE("[construct an address from bytes of different network type]", "[address_v4.ctor]") {
  ...
  CHECK((!normal.is_loopback() && !normal.is_multicast() && !normal.is_unspecified()));
}

TEST_CASE("[construct an address from unsigned integer in host byte order]", "[address_v4.ctor]") {
...
TEST_CASE("[copy constructor]", "[address_v4.ctor]") {
  ...
  CHECK(
      (!normal_copy.is_loopback() && !normal_copy.is_multicast() && !normal_copy.is_unspecified()));
...
TEST_CASE("[native_address should return sockaddr_storage]", "[address_v4.native_address]") {
  address_v4 normal{uint_ip[3]};
...
TEST_CASE("[pass well-formed address to make_address_v4 should work]",
  ...
  CHECK(make_address_v4("127.0.0.1").to_string() == "127.0.0.1");
  CHECK(make_address_v4("120.121.122.123").to_string() == "120.121.122.123");
  CHECK(make_address_v4(std::string_view{"120.121.122.123"}).to_string() == "120.121.122.123");
  CHECK(make_address_v4(std::string{"120.121.122.123"}).to_string() == "120.121.122.123");
  CHECK(make_address_v4(uint_ip[3]).to_string() == "120.121.122.123");
  CHECK(make_address_v4(address_v4::bytes_type{120, 121, 122, 123}).to_string()
        == "120.121.122.123");
}

TEST_CASE("[pass ill-formed address to make_address_v4 should return unspecified]",
          "[address_v4.make_address_v4]") {
