...
TEST_CASE("[construct socket_recv_some_op using std::string and ip::tcp]",
  ...
  std::string buf(1024, ' ');
  using operation =
      stdexec::__t<epoll_context::socket_recv_some_op<size_receiver,  //
                                                      net::ip::tcp,   //
                                                      mutable_buffer>>;
...
TEST_CASE("[construct socket_recv_some_op using buffer sequence and ip::tcp]",
  ...
  using operation = stdexec::__t<
      epoll_context::socket_recv_some_op<size_receiver,  //
                                         net::ip::tcp,   //
                                         std::vector<mutable_buffer>>>;

  using bufs_t =
      buffer_sequence_adapter<mutable_buffer, vector<mutable_buffer>>;
...
TEST_CASE("[construct sender using std::string and ip::tcp::socket]",
          "[recv_some_sender.ctor]") {
  epoll_context ctx{};
  net::ip::tcp::socket socket{ctx};
  std::string buf(1024, ' ');
  stdexec::__t<recv_some_sender<net::ip::tcp, mutable_buffer>> s{
...
TEST_CASE("[recv_some_sender::__t should satisfy stdexec::sender]",
          "[recv_some_sender.concept]") {
  CHECK(stdexec::sender<
        stdexec::__t<recv_some_sender<net::ip::tcp, mutable_buffer>>>);
}
...
TEST_CASE("[CPO: `start` performed operation]",
          "[epoll_socket_recv_some_op.start]") {
  epoll_context ctx{};
  std::jthread io_thread([&]() { ctx.run(); });
